...
const ProductOverView = () => {
  ...
  const deleteProductFunc = async () => {
    const isOkay = confirm("Product will be deleted permanently, are you sure ?")
    if (isOkay) {
      setDelSpin(true)
      try {
        ...
        {spin ?
          ...
          <div className='lg:flex lg:items-start lg:justify-between'>
            ...
            <div className='mt-3 lg:mt-0 lg:w-[45%]'>
              ...
              <h5 className='text-blue-800 text-[1.1rem]'>Item Name : <span className='text-black font-semibold capitalize'>{product.itemName}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Cost : <span className='text-black font-semibold te'>₹{product.itemCost}</span></h5>

              <hr className='border  border-gray-200 mb-2 mt-1' />


              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Half Kg Cost : <span className='text-black font-semibold'>₹{product.itemHalfKgCost}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Kg Cost : <span className='text-black font-semibold'>₹{product.itemKgCost}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Offer Cost : <span className='text-black font-semibold'>₹{product.offerCost}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Minimum Order Qty : <span className='text-black font-semibold'>{product.minOrderQty}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item description : <span className='text-black font-semibold'>{product.itemDescription}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Stock : <span className='text-black font-semibold'>{product.itemStock}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item Category : <span className='text-black font-semibold'>{product.itemCategory}</span></h5>
              <h5 className='text-blue-800 mt-2 text-[1.1rem]'>Item SubCategory : <span className='text-black font-semibold'>{product.itemSubCategory}</span></h5>
              <div className='flex items-start flex-wrap gap-1 mt-2'>

                <h5 className='text-blue-800 text-nowrap text-[1.1rem] '>Item Weight Available :
                </h5>
                {product?.itemWeight?.map((item) => (
                  <h5 key={item} className='text-white px-2 rounded bg-indigo-600   font-semibold  mr-2'>{item} grams </h5>
                ))}
              </div>
              <div className='flex items-start flex-wrap gap-1 mt-2'>

                <h5 className='text-blue-800  text-[1.1rem] text-nowrap'>Product Tags :
                </h5>
                {product?.productTags?.map((item) => (
                  <span key={item} className='text-white px-2 rounded bg-orange-600  font-semibold mr-2'>{item}</span>
                ))}
              </div>
